...
// Build query
$query = "SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.on_sale = 1 AND p.original_price > p.price";
$count_query = "SELECT COUNT(*) as total FROM products WHERE on_sale = 1 AND original_price > price";
...
switch ($sort) {
    ...
    case 'discount':
    default:
        $query .= " ORDER BY ((p.original_price - p.price) / p.original_price) DESC";
        break;
}
...
<head>
    ...
    <title>Special Deals - ElectroShop</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="deals-hero">
            <div class="deals-hero-content">
                <h1>Special Deals</h1>
                <p>Save big on our best electronics with these limited-time offers!</p>
            </div>
        </div>
        ...
        <div class="products-header">
            <div class="products-sort">
                ...
                <select id="sort" onchange="window.location.href=this.value">
                    <option value="?sort=discount" <?php echo $sort === 'discount' ? 'selected' : ''; ?>>Biggest Discount</option>
    ...
    <style>
        .deals-hero {
            background-image: linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url("images/deals-bg.jpg");
